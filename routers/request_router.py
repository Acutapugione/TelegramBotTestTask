...
from shared.keyboards import frequency_keyboard, stop_keyboard
...
async def handle_sending_requests(callback_query: CallbackQuery, state: FSMContext):
    ...
    if callback_query.data == "start_requesting":
        ...
        active_sessions_count = len(active_sessions.get(user_id, []))

        if user_status == EUserStatus.DEMO and active_sessions_count >= 1:
            await callback_query.message.answer(
                "❌ В демо статусі доступна можливість запускати одночасно лише одну сесію."
            )
        elif (
            user_status in (EUserStatus.UNLIMITED, EUserStatus.MAX)
            and active_sessions_count >= 3
        ):
            await callback_query.message.answer(
...
async def initiate_request(message: Message, state: FSMContext, user_id=None):
    ...
    if user_data.get("status") == EUserStatus.DEMO and is_demo_limit_reached(user_id):
        await message.answer(
    ...
    if user_data.get("status") == EUserStatus.DEMO:
        ...
        await message.answer(
            ...
            reply_markup=ReplyKeyboardMarkup(
                keyboard=[[KeyboardButton(text="❌ Відмінити запуск")]],
...
@request_router.message(lambda message: message.text == "❌ Відмінити запуск")
async def cancel_request(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Запуск відправки заяву відмінено", reply_markup=get_start_keyboard(user_id)
    )
...
async def handle_url(message: Message, state: FSMContext):
    ...
    if is_valid_url(url):
        ...
        user_active_sessions = active_sessions.get(user_id, [])
        user_active_domains = active_domains.setdefault(user_id, {})
        ...
        if active_sessions.get(user_id):
            active_sessions[user_id].append(url)
        else:
            active_sessions[user_id] = [
                url,
            ]
        user_active_domains[domain] = url
...
async def request_loop(user_id, frequency, url, state, message, duration=None):
    ...
    if (
        ...
        active_sessions[user_id].pop(-1)
        print(active_domains[user_id])
        active_domains[user_id].pop(list(active_domains[user_id].keys())[-1])

    await state.set_state(UserState.waiting_for_start)
    update_applications_sent(
        user_id, users[user_id]["applications_sent"] + request_counter
    )
...
async def stop_sending(message: Message, state: FSMContext):
    ...
    active_domains.pop(user_id, None)
    total_requests = 0
    ...
    if user_id in users:
        update_applications_sent(
            user_id, users[user_id]["applications_sent"] + total_requests
        )
